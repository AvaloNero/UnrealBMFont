...
Packed-channel pages render through a per-page `UMaterialInstanceDynamic` created from the asset's packed render material (default: the plugin's `M_BMFontPacked`). The material instance receives the page texture and the channel weights computed from the descriptor's channel metadata, and Slate's vertex color continues to apply tint and shadow. Non-packed pages keep the plain texture path.

## Text model

The core accepts plain text and maps each code point through the BMFont layout and paint primitives. Rich Text markup lives in an optional adapter: `UBMFontRichTextBlock` contributes a Slate decorator that converts matching runs into BMFont runs measured and painted with the same glyph metrics. Each rich-text run is laid out independently, so kerning does not cross run boundaries.
