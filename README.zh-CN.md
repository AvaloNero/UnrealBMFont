...
插件当前版本为 Beta `0.1.0`。Runtime API 已保持精简可用，但不会宣称支持尚未验证的引擎版本和平台。
...
- 提供 `UBMFontAsset`、`UBMFontText` 和底层 `SBMFontText`。
...
BMFont 保存的是已经栅格化的字形矩形，不是字体塑形系统。当前版本适用于拉丁文、CJK、数字、图标和其它从左到右的预制字形集；不提供双向排版、OpenType shaping、连字或 Rich Text 装饰器 API。Packed-channel 图集可以被解析和诊断，但导入器会拒绝它，因为当前控件尚无可靠的通道选择材质路径。
